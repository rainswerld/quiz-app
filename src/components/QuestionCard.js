...
function QuestionCard() {
  ...
  const shuffle = (data) => {
    const rando = getRandomInt(4)
    const ansArr = data.incorrect
    ansArr.splice(rando, 0, data.correct)
    console.log(ansArr.length)
    return ansArr
    // data.incorrect.push(data.correct)
    // return data.incorrect.sort(() => Math.random() - 0.5)
  }

  let answers
  const questions = quiz.map(data => {
    // const allAnswers = data.incorrect.push(data.correct)
    answers = shuffle(data)

    return (
      <Card key={data.correct} className="card-style">
        <Card.Title className="card-title">{data.question}</Card.Title>
        <Card.Body>
          <ListGroup>
            {
              answers.map(answer => <ListGroup.Item>{answer}</ListGroup.Item>)
            }
          </ListGroup>
        </Card.Body>
      </Card>
